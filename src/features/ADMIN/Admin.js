...
export function Admin () {
    ...
    return isShowed ? <div className="admin-div">
        <button onClick={() => setIsShowed(() => !isShowed)}>Hide</button>
        <input onKeyDown={changeTimingOnServer} onChange={(e) => setTiming(() => e.target.value)} value={timing} type="number" />
    ...
    </div> : <div className="admin-div"><button onClick={() => setIsShowed(() => !isShowed)} >Open</button></div>
    }
}